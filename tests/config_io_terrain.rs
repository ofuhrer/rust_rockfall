...
fn validation_output_mode_rebuildable_reduced_output_writes_builder_facing_outputs() {
    ...
    let manifest_json: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&manifest).unwrap()).unwrap();
    assert_eq!(
        manifest_json["validation_output_mode"],
    ...
    assert_eq!(
        ...
        vec![
            ...
            "impact_events_csv".to_string(),
            "ensemble_deposition".to_string(),
            "ensemble_stop_state".to_string(),
            "trajectory_metadata".to_string(),
...
#[test]
fn terrain_class_impact_sidecar_records_parquet_only_context() {
    let case_path = temp_path("terrain_class_parquet_impact_sidecar_case.yaml");
    let diagnostics = temp_path("terrain_class_parquet_impact_sidecar_metrics.json");
    let manifest = temp_path("terrain_class_parquet_impact_sidecar_manifest.json");
    let impact_parquet = temp_path("terrain_class_parquet_impacts.parquet");
    let impact_material_dir = impact_parquet.with_file_name(format!(
        "{}_terrain_material",
        impact_parquet.file_stem().unwrap().to_string_lossy()
    ));
    let _ = fs::remove_file(&impact_parquet);
    let _ = fs::remove_dir_all(&impact_material_dir);
    fs::write(
        &case_path,
        format!(
            r#"case_id: terrain_class_parquet_impact_sidecar
title: Terrain class parquet impact sidecar
level: 3
description: Temporary case that writes per-impact terrain/material context beside Parquet.
terrain:
  type: ascii_dem_clamped
  path: validation/data/processed/swisstopo_pilot/swissalti3d_pilot_crop.asc
  metadata_path: validation/data/processed/swisstopo_pilot/swissalti3d_pilot_metadata.yaml
...
"#,
    ...
    let first_row = read_first_csv_row(&material_files[0]);
    assert_eq!(first_row["terrain_material_context_status"], "classified");
    assert!(first_row["active_parameter_override_values"].contains("restitution_n"));
    let manifest_json: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&manifest).unwrap()).unwrap();
    assert!(manifest_json["outputs"]
        ...
        .unwrap()
        .iter()
        .any(|entry| entry["kind"] == "ensemble_impact_events" && entry["format"] == "parquet"));
    let impact_material_manifest = manifest_json["outputs"]
        .as_array()
        .unwrap()
        .iter()
        .find(|entry| entry["kind"] == "ensemble_impact_terrain_material")
        .expect("parquet companion impact terrain/material sidecar manifest");
    assert_eq!(
        impact_material_manifest["schema_version"],
        "impact_terrain_material_table_v1"
    );
    ...
    fs::remove_file(diagnostics).unwrap();
    fs::remove_file(manifest).unwrap();
    fs::remove_file(impact_parquet).unwrap();
    fs::remove_dir_all(impact_material_dir).unwrap();
}
