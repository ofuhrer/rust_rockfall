...
class TerrainReleaseZoneCandidateMetricsTests(unittest.TestCase):
    def test_committed_tschamut_inputs_produce_deterministic_candidate_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("candidate_size_summary", first_feature["properties"])
            self.assertIn("candidate_separation_summary", first_feature["properties"])
            self.assertIn("candidate_context_exclusion_summary", first_feature["properties"])
            self.assertIn("candidate_review_status_summary", first_feature["properties"])
            self.assertIn("smoothed slope range", first_feature["properties"]["candidate_slope_band_summary"])
            self.assertIn("3x3 local relief range", first_feature["properties"]["candidate_local_relief_summary"])
            self.assertIn("connected cells", first_feature["properties"]["candidate_size_summary"])
            self.assertIn("frozen source-zone footprint", first_feature["properties"]["candidate_context_exclusion_summary"])
            self.assertIn("review_decision=needs_field_review", first_feature["properties"]["candidate_review_status_summary"])
            ...
            self.assertIn("candidate_separation_summary", review_geojson["features"][0]["properties"])
            self.assertIn("candidate_review_status_summary", review_geojson["features"][0]["properties"])
    ...
    def test_fixture_terrain_package_fields_feed_candidate_report(self) -> None:
        ...
        self.assertEqual(report["terrain_preprocessing"]["terrain_preprocessing_status"], "ready_with_warnings")
        self.assertEqual(report["terrain_preprocessing"]["terrain_domain_qa"]["aoi_containment_status"], "missing_site_extent")
        self.assertEqual(report["terrain_inputs"]["terrain_preprocessing_status"], "ready_with_warnings")
    ...
    def test_review_apply_edits_candidates_and_validates_provenance(self) -> None:
        ...
        self.assertIn("candidate_slope_band_summary", report["selection_manifest"]["candidate_selection_rows"][0])
        self.assertIn("candidate_review_status_summary", report["selection_manifest"]["candidate_selection_rows"][0])
        ...
        self.assertIn("review_decision=accepted", geojson["features"][0]["properties"]["candidate_review_status_summary"])
