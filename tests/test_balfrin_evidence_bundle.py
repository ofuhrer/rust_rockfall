...
class BalfrinEvidenceBundleTests(unittest.TestCase):
    ...
    def test_current_report_is_measured_and_tracks_section_provenance(self) -> None:
        ...
        self.assertEqual(report["probe_metrics"]["metrics_completion_source"], "new_metrics_completion_rerun")
        self.assertEqual(report["probe_metrics"]["metrics_completion_outcome"], "measured")
        self.assertEqual(report["probe_metrics"]["metrics_evidence_state"]["memory_peak_mb"], 5.4375)
        self.assertEqual(report["probe_metrics"]["metrics_evidence_state"]["validation_output"]["file_count"], 130)
        self.assertEqual(report["probe_metrics"]["metrics_evidence_state"]["hazard_output"]["bytes"], 273194249)
        self.assertEqual(report["probe_metrics"]["metrics_evidence_state"]["slurm"]["job_id"], "4339889")
        self.assertTrue(report["probe_metrics"]["metrics_evidence_state"]["preservation_checked"])
        self.assertEqual(report["gis_cog_scope_report"]["scope_status"], "full_scope")
        self.assertEqual(report["gis_cog_scope_report"]["scope_delta_status"], "parity_match")
        ...
        self.assertEqual(latest_gate["run_id"], "tschamut_public_balfrin_four_zone_hazard_tb619_20260527")
        ...
        self.assertEqual(hazard["task_id"], "TB-619")
        self.assertEqual(hazard["slurm_job_id"], "4372656")
        self.assertEqual(hazard["release_zone_count"], 4)
        self.assertEqual(hazard["hazard_output_bytes"], 31439445)
        self.assertEqual(hazard["conditional_curve_row_count"], 729600)
        self.assertEqual(hazard["comparison_baseline"]["task_id"], "TB-603")
        self.assertEqual(hazard["comparison_baseline"]["slurm_job_id"], "4372309")
    ...
    def test_current_multi_zone_evidence_records_latest_regional_split_branch(self) -> None:
        ...
        self.assertEqual(multi_zone["status"], "measured")
        self.assertEqual(multi_zone["evidence_type"], "measured")
        self.assertEqual(multi_zone["task_id"], "TB-565")
        self.assertEqual(multi_zone["root_class"], "measured_multi_zone_balfrin_root")
        self.assertEqual(multi_zone["slurm_job_id"], "4367244")
        self.assertEqual(multi_zone["slurm_state"], "COMPLETED")
        self.assertEqual(multi_zone["exit_code"], "0:0")
        self.assertEqual(multi_zone["elapsed"], "00:00:24")
        self.assertEqual(multi_zone["memory_peak_mb"], 172.921875)
        self.assertEqual(multi_zone["validation_output_file_count"], 130)
        self.assertEqual(multi_zone["validation_output_bytes"], 34565330)
        self.assertEqual(multi_zone["hazard_output_file_count"], 57)
        self.assertEqual(multi_zone["hazard_output_bytes"], 57670915)
        self.assertEqual(multi_zone["conditional_curve_row_count"], 729600)
        self.assertEqual(multi_zone["release_zone_count"], 12)
        self.assertTrue(multi_zone["metrics_json_promoted"])
        self.assertTrue(multi_zone["preservation_checked"])
        self.assertTrue(multi_zone["preservation_gate_promoted"])
        self.assertTrue(multi_zone["post_run_collector_promoted"])
        self.assertEqual(multi_zone["preservation_gate_status"], "ready_for_demonstration_evidence")
        self.assertEqual(multi_zone["required_run_root_entries_status"], "complete")
        self.assertEqual(multi_zone["output_family_status"], "sufficient")
        self.assertEqual(multi_zone["output_budget_audit_status"], "blocked_missing_replay_artifacts")
        self.assertEqual(multi_zone["latest_prior_measured_task"], "TB-557")
        self.assertEqual(multi_zone["next_blocker"], "none")
        self.assertIn("Measured multi-zone Balfrin evidence is present", multi_zone["summary"])
