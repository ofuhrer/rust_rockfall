...
class AoiHazardMapPackagerTests(unittest.TestCase):
    def test_fixture_hazard_outputs_package_into_cog_ready_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pilot_manifest = json.loads((output_root / PILOT_MANIFEST.name).read_text(encoding="utf-8"))
            ...
            self.assertEqual(manifest["schema_version"], "aoi_hazard_map_package_v1")
            self.assertEqual(manifest["operational_status"], "research_diagnostic")
            ...
            self.assertEqual(manifest["qgis_style_assets"]["schema_version"], "aoi_qgis_style_bundle_v1")
            self.assertEqual(manifest["qgis_style_assets"]["status"], "ready")
            ...
            self.assertEqual(manifest["review_ready_layer_order"][0]["layer_name"], "release_zone")
            self.assertEqual(manifest["review_ready_layer_order"][0]["display_name"], "Release zone")
            self.assertEqual(manifest["review_ready_layer_order"][0]["crs_authid"], "EPSG:2056")
            self.assertTrue(all(entry["review_layer"]["crs_authid"] == "EPSG:2056" for entry in manifest["layer_inventory"]))
            self.assertTrue(all(entry["review_layer"]["display_name"] for entry in manifest["layer_inventory"]))
            self.assertTrue(all(entry["review_layer"]["qgis_style_status"] == "ready" for entry in manifest["layer_inventory"]))
            self.assertTrue((output_root / "styles" / "aoi_qgis_style_bundle.json").exists())
            self.assertTrue((output_root / "styles" / "conditional_reach_probability.qml").exists())
            self.assertTrue((output_root / "styles" / "scenario_families.qml").exists())
            self.assertEqual(manifest["raster_outputs"][0]["qgis_style"]["status"], "ready")
            self.assertTrue(all(entry["qgis_style"]["status"] == "ready" for entry in manifest["raster_outputs"]))
            self.assertEqual(manifest["vector_overlays"][0]["qgis_style"]["style_id"], "candidate_source_zone")
            self.assertEqual(manifest["vector_overlays"][1]["qgis_style"]["style_id"], "scenario_families")
            ...
            self.assertTrue(pilot_manifest["hazard_manifest_paths"])
            self.assertEqual(len(pilot_manifest["raster_outputs"]), len(manifest["raster_outputs"]))
            self.assertTrue(all(entry["cloud_optimized"] for entry in pilot_manifest["raster_outputs"]))
            ...
            self.assertIn("Layer inventory", review_html)
            self.assertIn("Conditional semantics", review_html)
    ...
    def test_cog_conversion_failure_reports_cog_blocked_and_falls_back(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["review_surface_status"], "review_ready_with_warnings")
            self.assertEqual(report["review_surface_first_blocker"]["code"], "missing_context_layers")
