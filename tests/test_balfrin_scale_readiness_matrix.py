...
class BalfrinScaleReadinessMatrixTests(unittest.TestCase):
    def test_build_report_composes_the_authoritative_baseline_matrix(self) -> None:
        ...
        self.assertIn("TB-682 brackets larger hazard-output pressure", report["summary"])
        ...
        self.assertEqual(report["hazard_throughput_status"]["job_id"], "4379134")
        self.assertEqual(report["hazard_throughput_status"]["release_zone_count"], 24)
        self.assertEqual(
        ...
        self.assertIsNone(report["hazard_throughput_status"]["validation_output_file_count"])
        self.assertIsNone(report["hazard_throughput_status"]["validation_output_bytes"])
        self.assertEqual(report["hazard_throughput_status"]["hazard_output_file_count"], 29)
        self.assertEqual(report["hazard_throughput_status"]["hazard_output_bytes"], 1169964)
        ...
        output_ladder = report["hazard_throughput_status"]["hazard_output_pressure_ladder"]
        self.assertEqual(output_ladder["task_id"], "TB-682")
        self.assertEqual(output_ladder["run_count"], 3)
        self.assertEqual(output_ladder["release_zone_counts"], [96, 192, 384])
        self.assertEqual(output_ladder["largest_measured_output_bytes_safe_release_zone_count"], 192)
        self.assertEqual(output_ladder["largest_measured_output_bytes_safe_job_id"], "4379388")
        self.assertEqual(output_ladder["largest_measured_output_bytes_safe_hazard_output_bytes"], 1353399)
        self.assertEqual(output_ladder["largest_measured_output_bytes_safe_manifest_bytes"], 198522)
        self.assertEqual(output_ladder["first_measured_output_byte_blocked_release_zone_count"], 384)
        self.assertEqual(output_ladder["first_measured_output_byte_blocked_job_id"], "4379371")
        self.assertEqual(output_ladder["first_measured_output_byte_blocked_hazard_output_bytes"], 1536400)
        self.assertEqual(output_ladder["status"], "measured_ladder_bracketed_by_output_byte_budget")
        self.assertIn("192 release zones", output_ladder["next_safe_size"])
