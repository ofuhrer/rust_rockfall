...
This catalog lists the current `v0.6.0` verification and validation cases. These cases verify the independent simulator’s implemented equations and diagnostics; they do not claim agreement with RAMMS::ROCKFALL or operational hazard validity.
...
- `validation_swissalti3d_release_zone_terrain_classes_pilot`: same tiny Swiss terrain and source-area fixtures with an opt-in aligned categorical terrain/material-class raster. It verifies class-grid compatibility, class-dependent parameter lookup, class coverage summaries, and `run_manifest_v1` terrain-class provenance without changing global defaults.
...
- `validation_chant_sura_contact_roughness`: same DEM-backed contact fixture with deterministic opt-in `stochastic_contact_v1`; roughness is not calibrated.
- `validation_chant_sura_contact_scarring`: same DEM-backed contact fixture with opt-in `scarring_contact_v1` using the current impact-level calibration parameter set for exploratory model comparison only.
- `validation_chant_sura_contact_extended`: extended DEM-backed Chant Sura contact fixture using five source trajectories, 16 local-time-reset trajectory segments, and 11 segment-boundary contact/rebound proxies that remain inside the RF16 DEM crop.
...
Real-world validation remains partial and qualitative for v0.6.0 because the simulator lacks dynamic block-shape effects, advanced contact, calibrated scarring with drag torque/slip-dependent friction, roughness calibration, forest interaction, fragmentation, and production field DEM workflows.
