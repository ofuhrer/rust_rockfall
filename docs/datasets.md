...
The Chant Sura contact-validation fixtures now include the original two-event
RF16W200r1 subset, an extended model-selection subset with 16 segments and 11
segment-boundary contact/rebound proxies, and a held-out subset with 15 segments
and 9 contact/rebound proxies. All are constrained to the small RF16 DEM crop.
The split is intended for contact-model comparison only; it does not change the
separation between validation and calibration.
