...
- The decision remains `defer` because the task boundary forbids real downloads
  ...
  site. SWISSIMAGE is now staged, but the remaining public-context rows stay
  deferred and the package is still not ready for a second site.
...
- promote the already present real raw cache files into site-specific ignored context roots for `swisstlm3d` metadata, `swissSURFACE3D Raster`, and `swissBUILDINGS3D`
