...
Terrain access is abstracted through the `Terrain` trait. Analytic terrain remains unchanged. ESRI ASCII grids follow the standard GIS convention: `xllcorner` and `yllcorner` define the outer lower-left cell corner, raster values are sampled at cell centers, and metadata extents describe the full outer footprint. Strict ESRI ASCII grids use bilinear interpolation over the cell-center domain and intentionally return a terrain error on out-of-bounds or nodata access through `try_height`, `try_normal`, and the normal simulation entry points. The legacy infallible `height`/`normal` helpers are retained for low-level tests and simple terrain use, but new DEM-facing runtime code must use `try_height`/`try_normal` and the fallible simulation path. The module-level ownership and panic/error boundary are summarized in `docs/architecture_boundaries.md`.

ESRI ASCII `NODATA_value` cells are not interpolated as elevations in strict DEM mode. Any bilinear query touching a nodata or non-finite corner returns a terrain error through `try_height`. The clamped DEM variant clamps out-of-domain queries to the nearest cell-center domain and, if the interpolation stencil touches nodata, falls back to the nearest valid finite cell value. This keeps limited validation patches from crashing at raster holes while making the extrapolation policy explicit; it is not a full GIS nodata-repair workflow.
